...
The `funmediation` package must be installed before using it.  We plan to release it on the CRAN archive, so that it can be installed using the command 
```{r,eval=FALSE}
install.packages("funmediation")
````

Another option is to install it from the GitHub repository as follows:
